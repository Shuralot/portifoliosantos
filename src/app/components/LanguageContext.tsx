      ...
      list: [
        {
          ...
          description: "Party game multiplayer em tempo real construído com arquitetura orientada a eventos (Event-Driven) sobre WebSockets (Socket.IO). Desenvolvido para resolver o suporte morfológico ao Português Brasileiro (PT-BR) com sincronização de baixa latência entre PC, mobile e modo TV/telão.\n\nEngenharia & Decisões Arquiteturais:\n• Motor Léxico O(1) (403k+ Palavras): Validação em tempo de execução via Sets/HashMaps (<1ms de resposta), compilando léxico culto, formas verbais flexionadas, plurais e pipeline de normalização Unicode NFD tolerante a acentos e diacríticos.\n• Indexador de Prompts & Balanceamento Cognitivo: Pré-indexação de N-gramas (2 e 3 letras) com cálculo de densidade de soluções válidas por nível e busca reversa pós-explosão de termos reais do cotidiano.\n• Arquitetura Autoritativa & Live Typing: Máquina de Estados Finitos (FSM) no servidor que elimina trapaças e realiza stream em tempo real da digitação ativa para espectadores sem sobrecarregar a rede.\n• Áudio 100% Procedural (Web Audio API): Zero dependência de arquivos de áudio externos (0 KB). Tique-taque dinâmico e explosões sintetizadas em tempo real via ruído branco, filtros passa-baixa e envelopes de sub-bass.\n• Zero-Config Networking: Integração com Cloudflare Tunnels e gerador dinâmico de QR Code no terminal/web para entrada instantânea de múltiplos celulares sem liberação de portas (NAT traversal).\n• Modos Flexíveis: Suporte a Modo TV/Telão (jogadores usam smartphones como controle) e Modo Solo Arcade contra o relógio.",
      ...
      list: [
        {
          ...
          description: "Real-time multiplayer party game built with an event-driven architecture over WebSockets (Socket.IO). Engineered to solve the morphological limitations of classic word games by implementing a high-performance in-memory lexical engine tailored for Brazilian Portuguese (PT-BR) with synchronized low latency across PC, mobile, and TV.\n\nEngineering & Architectural Highlights:\n• O(1) Lexical Engine (403k+ Words): Sub-millisecond (<1ms) runtime validation using in-memory Sets/HashMaps, indexing cultured vocabulary, inflected verbs, plurals, and Unicode NFD normalization for accent-tolerant input.\n• N-gram Prompt Indexer & Cognitive Balance: Pre-indexes 2- and 3-letter n-grams calculating solution density across difficulty tiers, featuring post-round reverse lookups for missed everyday solutions.\n• Authoritative Server & Live Typing: Server-side Finite State Machine (FSM) eliminating cheating and streaming live keystrokes to spectators with sanitized payloads.\n• 100% Procedural Audio (Web Audio API): Zero external audio assets (0 KB). Real-time synthesized ticking and explosions combining white noise buffers, low-pass filters, and exponential sub-bass decay.\n• Zero-Config Networking: Automated Cloudflare Tunnel integration and dynamic QR Code generation (terminal & web) for frictionless smartphone room entry across Wi-Fi or public internet without router port forwarding.\n• Multi-Display Flexibility: Dedicated Big-screen / TV Mode designed for living room projections while smartphones act as game controllers, plus a Solo Arcade mode.",
          tags: ["Node.js", "Socket.IO", "WebSockets", "Web Audio API", "Event-Driven", "JavaScript", "Cloudflare Tunnels", "O(1) Engine"],
          github: "https://github.com/Shuralot/Batatoom"
        },
        {
